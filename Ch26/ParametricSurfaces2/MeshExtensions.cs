    ...
    double cubeThickness = 0.102)
{
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
        ...
        public static void AddTrefoilKnot(this MeshGeometry3D mesh,
            Point3D center, double R, int numT, int numU, bool smooth = false,
            HashSet<Edge> edges = null, double thickness = 0.1)
        {
            // Make a point dictionary if needed.
            Dictionary<Point3D, int> pointDict = null;
            ...
            // Generate points along the central curve.
            Vector3D up = D3.YVector();
            double dt = 2 * Math.PI / numT;
            double du = 2 * Math.PI / numU;
            double t = 0;
            Point3D[] centerPts = new Point3D[numT + 1];
            Vector3D[] vxs = new Vector3D[numT + 1];
            Vector3D[] vzs = new Vector3D[numT + 1];
            for (int it = 0; it <= numT; it++)
            {
                Vector3D vx, vz;
                centerPts[it] = TrefoilKnotCenterPoint(center, t, ref up, out vx, out vz);
                vxs[it] = vx;
                vzs[it] = vz;
                t += dt;
            }

            // Generate the surface pieces.
            t = 0;
            for (int it = 0; it < numT; it++)
            {
                int it1 = it + 1;
                ...
                for (int iu = 0; iu < numU; iu++)
                {
                    ...
                    Point3D[] points =
                    {
                        TrefoilKnotSurfacePoint(centerPts[it1], vxs[it1], vzs[it1], R, u),
                        TrefoilKnotSurfacePoint(centerPts[it1], vxs[it1], vzs[it1], R, u + du),
                        TrefoilKnotSurfacePoint(centerPts[it], vxs[it], vzs[it], R, u + du),
                        TrefoilKnotSurfacePoint(centerPts[it], vxs[it], vzs[it], R, u),
                    };
        ...
        // Return a point on a trefoil knot.
        private static Point3D TrefoilKnotCurvePoint(Point3D center, double t)
        {
            // Normal version.
        ...
        // Get a central point along the knot.
        private static Point3D TrefoilKnotCenterPoint(Point3D center,
            double t, ref Vector3D up, out Vector3D vx, out Vector3D vz)
        {
            const double dt = 0.01;
            Point3D p1 = TrefoilKnotCurvePoint(center, t);
            Point3D p2 = TrefoilKnotCurvePoint(center, t + dt);
            Vector3D v = p2 - p1;

            // Get perpendicular vectors.
            vz = Vector3D.CrossProduct(v, up);
            vx = Vector3D.CrossProduct(v, vz);
            up = -vx;
        ...
        // Return a point on a trefoil knot.
        private static Point3D TrefoilKnotSurfacePoint(Point3D centerPt,
            Vector3D vx, Vector3D vz, double R, double u)
        {
            return centerPt + vx * R * Math.Cos(u) + vz * R * Math.Sin(u);
        }

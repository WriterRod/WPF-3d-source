    ...
    double cubeThickness = 0.102)
{
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
        ...
        // Make a vector.
        public static void AddVector(this MeshGeometry3D mesh,
            Point3D point1, Point3D point2,
            double thickness = 0.1, double headThickness = 0.2, double headLength = 0.4)
        {
            // Get a vector between the points.
            Vector3D v = point2 - point1;
            ...
            // Make the segment box.
            mesh.AddBox(point1 - vx / 2 - vz / 2, vx, v, vz);

            // Add a pyramid.
            vx *= headThickness / 2 / vx.Length;
            vz *= headThickness / 2 / vz.Length;
            Point3D[] pgon =
            {
                point2 + vx + vz,
                point2 + vx - vz,
            ...
            v *= headLength / v.Length;
            mesh.AddPyramid(point2, pgon, v);
        }

        // Make models holding vectors parallel to the coordinate axes
        // and add them to the Model3DGroup.
        public static void MakeAxisVectors(Model3DGroup group,
            out List<MeshGeometry3D> meshes, out List<GeometryModel3D> models,
            Point3D point, double length = 1, double thickness = 0.1,
            double headThickness = 0.4, double headLength = 0.4)
        {
            models = new List<GeometryModel3D>();
            meshes = new List<MeshGeometry3D>();

            MeshGeometry3D xmesh = new MeshGeometry3D();
            xmesh.AddVector(point, point + D3.XVector(length), thickness, headThickness, headLength);
            meshes.Add(xmesh);
            models.Add(xmesh.MakeModel(Brushes.Red));

            MeshGeometry3D ymesh = new MeshGeometry3D();
            ymesh.AddVector(point, point + D3.YVector(length), thickness, headThickness, headLength);
            meshes.Add(ymesh);
            models.Add(ymesh.MakeModel(Brushes.Green));

            MeshGeometry3D zmesh = new MeshGeometry3D();
            zmesh.AddVector(point, point + D3.ZVector(length), thickness, headThickness, headLength);
            meshes.Add(zmesh);
            models.Add(zmesh.MakeModel(Brushes.Blue));

            MeshGeometry3D omesh = new MeshGeometry3D();
            double t = 0.75 * headThickness;
            Vector3D vx = D3.XVector(2 * t);
            Vector3D vy = D3.YVector(2 * t);
            Vector3D vz = D3.ZVector(2 * t);
            omesh.AddBox(point - new Vector3D(t, t, t), vx, vy, vz);
            meshes.Add(omesh);
            models.Add(omesh.MakeModel(Brushes.White));

            foreach (GeometryModel3D model in models) group.Children.Add(model);
        }
        ...
        // Make a surface y = F(x, z) without texture coordinates.
        ...
        // Make a surface y = F(x, z) with texture coordinates.
        public static void AddSurface(this MeshGeometry3D mesh,
            Func<double, double, Point3D> F,
            double xmin, double xmax, double umin, double umax, int numX,
            double zmin, double zmax, double vmin, double vmax, int numZ,
            bool smooth = false,
            HashSet<Edge> edges = null, double thickness = 0.1)
        {
            // Make a point dictionary if desired.
            ...
            // Generate the surface's points.
            double dx = (xmax - xmin) / numX;
            double du = (umax - umin) / numX;
            double dz = (zmax - zmin) / numZ;
            double dv = (vmax - vmin) / numZ;
            double x = xmin;
            double u = umin;
            for (int ix = 0; ix < numX; ix++)
            {

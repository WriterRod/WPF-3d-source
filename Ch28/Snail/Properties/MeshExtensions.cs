    ...
    double cubeThickness = 0.102)
{
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
        ...
        public static void AddTransformSurface(this MeshGeometry3D mesh,
            Point3D[] points, Transform3D trans, int num,
            ...
            HashSet<Edge> edges = null, double thickness = 0.1)
        {
            ...
            Point3D[] pts1 = new Point3D[points.Length];
            Point3D[] pts2 = new Point3D[points.Length];

            // Copy the original points into pts2.
            Array.Copy(points, pts2, numPoints);
            ...
            if (closeTransStart)
            {
                ...
                Array.Copy(points, pts, numPoints);
                Array.Reverse(pts);
                mesh.AddPolygon(pointDict: pointDict,
            ...
            if (closeRotStart)
            {
                Point3D[] pts = GetTransformPolygon(points[0], trans, num);
                mesh.AddPolygon(pointDict: pointDict,
                    edges: edges, thickness: thickness, points: pts);
            }
            if (closeRotEnd)
            {
                Point3D[] pts = GetTransformPolygon(points[numPoints - 1], trans, num);
                Array.Reverse(pts);
                mesh.AddPolygon(pointDict: pointDict,
                    edges: edges, thickness: thickness, points: pts);
            }

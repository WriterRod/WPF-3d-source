    ...
    double cubeThickness = 0.102)
{
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
        ...
        {
            // Find the indices of the three points on the path.
            int numPath = path.Length;
            int p0 = p1 - 1;
            int p2 = p1 + 1;

            // Find the path vector.
            Vector3D vA = path[p1] - path[p0];
            Vector3D vB = path[p2] - path[p1];
            Vector3D v = (vA + vB) / 2;

            // Find the polygon's plane vectors.
            Vector3D vz = Vector3D.CrossProduct(v, up);
            Vector3D vx = Vector3D.CrossProduct(v, vz);
            vz.Normalize();
            vx.Normalize();

...
namespace CircleChart
{
    public static class MeshExtensions
    {
        ...
        // If the point is already in the dictionary, return its index in the mesh.
        // If the point isn't in the dictionary, create it and add its index to the dictionary.
        // If texture coordinates or normals are included, create them, too.
        private static int PointIndex(this MeshGeometry3D mesh,
            Point3D point, int textureIndex, Point[] textureCoords = null,
            ...
            Dictionary<Point3D, int> pointDict = null)
        {
            ...
            if (textureCoords != null)
                mesh.TextureCoordinates.Add(textureCoords[textureIndex]);

            // Add the point's normal.
            if (normals != null)
                mesh.Normals.Add(normals[textureIndex]);
            ...
            params Point3D[] points)
        {
            ...
            {
                // Make a wireframe polygon.
                mesh.AddPolygonTriangles(pointDict, textureCoords, normals, points);
            }
        }
            ...
            params Point3D[] points)
        {
            ...
            indexA = mesh.PointIndex(roundedA, 0, textureCoords, normals, pointDict);

            Point3D roundedC = points[1].Round();
            indexC = mesh.PointIndex(roundedC, 1, textureCoords, normals, pointDict);
            ...
            for (int i = 2; i < points.Length; i++)
            {
                ...
                indexC = mesh.PointIndex(points[i].Round(), i, textureCoords, normals, pointDict);
            ...
            Point[] textureCoords = null, Vector3D[] normals = null)
        {
            ...
            // Make the polygon.
            mesh.AddPolygon(pointDict, edges, thickness, textureCoords, normals, points);
        }
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
            ...
            bool smoothSides = false, HashSet<Edge> edges = null, double thickness = 0.1)
        {
            ...
            for (int i = 0; i < numPoints; i++)
            {
                ...
                mesh.AddPolygon(pointDict, edges, thickness, null, null,
            ...
            mesh.AddPolygon(pointDict, edges, thickness, null, null, bottom);
        }
            ...
            Point[] textureCoords = null, Vector3D[] normals = null)
        {
            ...
            for (int ix = 0; ix < numX; ix++)
            {
                ...
                for (int iz = 0; iz < numZ; iz++)
                {
                    ...
                    Point3D p4 = F(x + dx, z);
                    mesh.AddPolygon(pointDict, edges, thickness,
                        textureCoords, normals, p1, p2, p3, p4);
                    z += dz;
                }
                x += dx;
            }
            ...
            Point[] textureCoords = null, Vector3D[] normals = null)
        {
            ...
            for (int ix = 0; ix < numX; ix++)
            {
                for (int iz = 0; iz < numZ; iz++)
                {
                    ...
                    mesh.AddPolygon(pointDict, edges, thickness, textureCoords, normals,
                        p1, p2, p3, p4);
                }
            }
        }
        ...
        #region Fit Mesh to Box

        // Transform the mesh to fit inside the given box.
        public static void FitToBox(this MeshGeometry3D mesh, Rect3D box)
        {
            // Get the transformation.
            Transform3DGroup group = GetBoxMapping(mesh.Bounds, box);

            // Apply the transformation.
            mesh.ApplyTransformation(group);
        }

        // Make a Transform3DGroup to map a box to another box without distortion.
        public static Transform3DGroup GetBoxMapping(Rect3D source, Rect3D dest)
        {
            // Translate to center the source at the origin.
            Transform3DGroup group = new Transform3DGroup();
            group.Children.Add(new TranslateTransform3D(
                -(source.X + source.SizeX / 2),
                -(source.Y + source.SizeY / 2),
                -(source.Z + source.SizeZ / 2)));

            // Calculate the scale factor.
            double xscale = dest.SizeX / source.SizeX;
            double yscale = dest.SizeY / source.SizeY;
            double zscale = dest.SizeZ / source.SizeZ;
            double scale = xscale;
            if (scale > yscale) scale = yscale;
            if (scale > zscale) scale = zscale;

            // Scale.
            group.Children.Add(new ScaleTransform3D(scale, scale, scale));

            // Translate to center in the desired box.
            group.Children.Add(new TranslateTransform3D(
                dest.X + dest.SizeX / 2,
                dest.Y + dest.SizeY / 2,
                dest.Z + dest.SizeZ / 2));
        ...
        #endregion Fit Mesh to Box

        #region Pie Slices

        // Make a pie slice counter clockwise as seen from above between angles
        // minTheta and maxTheta in degrees.
        public static void AddPieSlice(this MeshGeometry3D mesh,
            Point3D center, double height, double minTheta, double maxTheta,
            double r, int numSections)
        {
            // Center of the slice.
            Point3D topCorner = center + D3.YVector(height);
            Point3D botCorner = center;

            // Unit vectors in X and Z direction for sines and cosines.
            Vector3D vx = D3.XVector(r);
            Vector3D vz = D3.ZVector(-r);

            // Convert thetas into radians.
            minTheta *= Math.PI / 180;
            maxTheta *= Math.PI / 180;

            // Point dictionaries for smoothing.
            Dictionary<Point3D, int> topDict = new Dictionary<Point3D, int>();
            Dictionary<Point3D, int> botDict = new Dictionary<Point3D, int>();
            Dictionary<Point3D, int> outDict = new Dictionary<Point3D, int>();

            double dtheta = (maxTheta - minTheta) / numSections;
            double theta = minTheta;
            
            // Vector in the direction of theta.
            Vector3D v = vx * Math.Cos(theta) + vz * Math.Sin(theta);
            Point3D[] topPoints =
            {
                topCorner,
                topCorner,
            ...
            for (int i = 0; i < numSections; i++)
            {
                ...
                // Vector in the direction of theta.
                v = vx * Math.Cos(theta) + vz * Math.Sin(theta);

                // Top.
                topPoints[1] = topPoints[2];
                topPoints[2] = topCorner + v;
                mesh.AddPolygon(topDict, points: topPoints);

                // Bottom.
                botPoints[2] = botPoints[1];
                botPoints[1] = botCorner + v;
                mesh.AddPolygon(botDict, points: botPoints);

                // Outer face.
                outPoints[0] = outPoints[3];
                outPoints[1] = outPoints[2];
                outPoints[2] = botCorner + v;
                outPoints[3] = topCorner + v;
                mesh.AddPolygon(outDict, points: outPoints);
            }

            // Side1.
            Dictionary<Point3D, int> side1Dict = new Dictionary<Point3D, int>();
            v = vx * Math.Cos(minTheta) + vz * Math.Sin(minTheta);
            Point3D[] side1Points =
            {
                botCorner,
                botCorner + v,
            ...
            mesh.AddPolygon(side1Dict, points: side1Points);

            // Side2.
            Dictionary<Point3D, int> side2Dict = new Dictionary<Point3D, int>();
            v = vx * Math.Cos(maxTheta) + vz * Math.Sin(maxTheta);
            Point3D[] side2Points =
            {
                topCorner,
                topCorner + v,
            ...
            mesh.AddPolygon(side2Dict, points: side2Points);
        }

        // Make a circle slice counter clockwise as seen from above between angles
        // minTheta and maxTheta in degrees.
        public static void AddCircleSlice(this MeshGeometry3D mesh,
            Point3D center, double height, double minTheta, double maxTheta,
            double r1, double r2, int numSections)
        {
            // Center of the slice.
            Point3D topCorner = center + D3.YVector(height);
            Point3D botCorner = center;
            ...
            // Unit vectors in X and Z direction for sines and cosines.
            Vector3D v1x = D3.XVector(r1);
            Vector3D v1z = D3.ZVector(-r1);
            Vector3D v2x = D3.XVector(r2);
            Vector3D v2z = D3.ZVector(-r2);
            ...
            Dictionary<Point3D, int> botDict = new Dictionary<Point3D, int>();
            Dictionary<Point3D, int> outDict = new Dictionary<Point3D, int>();
            Dictionary<Point3D, int> inDict = new Dictionary<Point3D, int>();

            double dtheta = (maxTheta - minTheta) / numSections;
            double theta = minTheta;

            // Vector in the direction of theta.
            Vector3D v1 = v1x * Math.Cos(theta) + v1z * Math.Sin(theta);
            Vector3D v2 = v2x * Math.Cos(theta) + v2z * Math.Sin(theta);
            Point3D[] topPoints =
            {
                topCorner + v1,
                topCorner + v2,
            ...
            for (int i = 0; i < numSections; i++)
            {
                ...
                // Vector in the direction of theta.
                v1 = v1x * Math.Cos(theta) + v1z * Math.Sin(theta);
                v2 = v2x * Math.Cos(theta) + v2z * Math.Sin(theta);

                // Top.
                topPoints[0] = topPoints[3];
                topPoints[1] = topPoints[2];
                topPoints[3] = topCorner + v1;
                topPoints[2] = topCorner + v2;
                mesh.AddPolygon(topDict, points: topPoints);

                // Bottom.
                botPoints[0] = botPoints[3];
                botPoints[1] = botPoints[2];
                botPoints[2] = botCorner + v1;
                botPoints[3] = botCorner + v2;
                mesh.AddPolygon(botDict, points: botPoints);

                // Outer face.
                outPoints[0] = outPoints[3];
                outPoints[1] = outPoints[2];
                outPoints[2] = botCorner + v2;
                outPoints[3] = topCorner + v2;
                mesh.AddPolygon(outDict, points: outPoints);
                
                // Inner face.
                inPoints[0] = inPoints[3];
                inPoints[1] = inPoints[2];
                inPoints[2] = topCorner + v1;
                inPoints[3] = botCorner + v1;
                mesh.AddPolygon(inDict, points: inPoints);
            }
        }

        #endregion Pie Slices

    ...
    double cubeThickness = 0.102)
{
    ...
    group.Children.Add(mesh.MakeModel(Brushes.Black));
        ...
        // Make models to represent the coordinate axes.
        public static void AddAxisModels(this Model3DGroup group,
            double length = 4, double thickness = 0.1)
        {
            MeshGeometry3D xmesh = new MeshGeometry3D();
            xmesh.AddSegment(thickness, D3.Origin, D3.Origin + D3.XVector(length));
            group.Children.Add(xmesh.MakeModel(Brushes.Red));

            MeshGeometry3D ymesh = new MeshGeometry3D();
            ymesh.AddSegment(thickness, D3.Origin, D3.Origin + D3.YVector(length));
            group.Children.Add(ymesh.MakeModel(Brushes.Green));

            MeshGeometry3D zmesh = new MeshGeometry3D();
            zmesh.AddSegment(thickness, D3.Origin, D3.Origin + D3.ZVector(length));
            group.Children.Add(zmesh.MakeModel(Brushes.Blue));

            MeshGeometry3D omesh = new MeshGeometry3D();
            double t = -thickness * 0.6;
            Vector3D vx = D3.XVector(-2 * t);
            Vector3D vy = D3.YVector(-2 * t);
            Vector3D vz = D3.ZVector(-2 * t);
            omesh.AddBox(new Point3D(t, t, t), vx, vy, vz);
            group.Children.Add(omesh.MakeModel(Brushes.Black));
        }

...
namespace Interlocked
{
    public static partial class MeshExtensions
    {
            ...
            Point3D point1, Point3D point2, Point3D point3)
        {
            ...
            int index1 = mesh.FindOrCreatePoint(point1);
            int index2 = mesh.FindOrCreatePoint(point2);
            int index3 = mesh.FindOrCreatePoint(point3);
        ...
        private static int FindOrCreatePoint(this MeshGeometry3D mesh,
            Point3D point)
        {
            // Find the point if it's present.
            int index = mesh.FindPoint(point);
        ...
        // Find a point and return its index.
        private static int FindPoint(this MeshGeometry3D mesh, Point3D point)
        {
            // Find the point if it's present.
            for (int i = 0; i < mesh.Positions.Count; i++)
                if (PointEquals(point, mesh.Positions[i], Precision))
                    return i;
            return -1;
        }
        ...
        public delegate double FofXZ(double x, double z);
        public static void AddSurface(this MeshGeometry3D mesh,
            FofXZ F, double xmin, double xmax, double zmin, double zmax,
            double dx, double dz)
        {
